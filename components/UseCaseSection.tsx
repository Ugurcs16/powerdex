...
export function UseCaseSection() {
  return (
    <section className="border-y border-[#252A33] bg-[#151922]/50 py-20">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <p className={`text-xs font-semibold uppercase tracking-[0.2em] ${brandClasses.accent}`}>
          Kullanım Senaryoları
        </p>
        <h2 className="mt-3 text-3xl font-bold text-white sm:text-4xl">Her İş İçin Doğru Işık</h2>
          ...
          {useCases.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className="mt-4 inline-block rounded-md border border-[#252A33] bg-[#0B0D10] px-2 py-1 text-xs text-zinc-300">
                  {item.productTag}
                </span>
              </motion.article>
            );
