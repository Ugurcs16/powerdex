...
export function TrustBar() {
  return (
    <section className={`border-y ${brandClasses.border} ${brandClasses.surface}`}>
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-3 px-4 py-4 sm:grid-cols-2 sm:px-6 lg:grid-cols-5 lg:px-8">
        {trustItems.map((item, index) => {
          ...
          return (
            <div
              key={item}
              className={`flex items-center gap-3 rounded-lg border ${brandClasses.border} ${brandClasses.bg} p-3`}
            >
              <Icon className={`size-4 shrink-0 ${brandClasses.accent}`} />
              <p className={`text-xs ${brandClasses.textMuted}`}>{item}</p>
            </div>
          );
        })}
      </div>
